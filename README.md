...
## Компиляция
Всё, что нужно это установленный компилятор [Golang](https://golang.org), настроить ```GOPATH```, выполнить ```go get``` и ```go build```
В целом этот шаг нужен только в том случае, если Вам понадобится изменить логику работы API.
...
По-умолчанию обновление происходит каждые 10 минут, изменить это можно в ``serverConfig.json``, (ключ UpdateInterval) значение указывается в минутах.
...
Список досок и типов файлов по-умолчанию описаны в [данном файле](https://github.com/d0kur0/webm-api/blob/master/helpers/grabberSchemaHelper/GrabberSchema.go#L35).
...
При первом запуске создаётся файл ``serverConfig.json`` в котором описан порт по-умолчанию, его можно изменить и при следующем запуске сервера он будет висеть на новом порту.
